...
| Container/image security scanning | Stage 7A complete with measured baseline: 52 CVEs detected/baselined (2 CRITICAL openssl CVE-2026-31789, base-bump path identified), config scan 29 findings (KSV-0118 triage pending), gitleaks 0 findings, SPDX SBOM of built image, image policy 0 violations; supply-chain incident (GHSA-69fq-xp46-6x23) documented and bypassed via SHA pins | Experimental (detect+baseline; nothing BLOCKED) | yes (evidence-linked) | 7B: triage + policy-from-baseline gate |
