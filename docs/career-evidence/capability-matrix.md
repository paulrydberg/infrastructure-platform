...
| Container/image security scanning | 7A evidence + 7B verified remediation (CRITICAL 0, KSV-0118 0 on deployable manifests) + debt register; 7C policy fully MODELED and SIMULATED across 3 historical states (ADR-0005 refined: R1/R2/R4 + expiring-exception design) — shadow mode & enforcement NOT implemented | Tested (remediation verified; deterministic security policy ENFORCED in CI since eb451e2 — schema-validated JSON input, provenance-tagged verdicts, artifact-preserving gate, unit-proven blocking + rollback; live run verified) | yes (evidence-linked) | operational |
...
| Disaster recovery / reconstruction | reconstruction manifest v1.0.0 + deterministic runner; LEVEL 4 ACHIEVED: L4 GitOps-layer reconstruction + L6 periodically verified reconstruction (launchd-scheduled resource-gated disposable validation, evidence contract with non-collapsing failure taxonomy, historical comparison; 17 defects fixed with preserved evidence incl. evidence-authority invariant and two live resource-gate signal corrections; 11/11 + 21/21 deterministic tests) | Implemented + verified | yes (level4-final-evidence.json + failure-test evidence) | Level 5 DR / cloud |
