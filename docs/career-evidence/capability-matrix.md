...
| Container/image security scanning | 7A evidence + 7B verified remediation (CRITICAL 0, KSV-0118 0 on deployable manifests) + debt register; 7C policy fully MODELED and SIMULATED across 3 historical states (ADR-0005 refined: R1/R2/R4 + expiring-exception design) — shadow mode & enforcement NOT implemented | Tested (remediation verified; shadow evaluator hardened (schema validation, provenance, versioning, tested enforcement/rollback contract) and validated across 3 real CI cycles; enforcement not wired) | yes (evidence-linked) | 7C enforcement authorization gate |
