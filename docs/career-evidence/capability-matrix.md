...
| Container/image security scanning | 7A evidence + 7B verified remediation (CRITICAL 0, KSV-0118 0 on deployable manifests) + debt register; 7C policy DESIGNED AND SIMULATED against historical scan inventories (ADR-0005: R1/R2/R4 selected, raw-count rules rejected) — shadow mode & enforcement NOT implemented | Tested (remediation verified; policy simulated, not wired into CI) | yes (evidence-linked) | 7C shadow-mode authorization gate |
