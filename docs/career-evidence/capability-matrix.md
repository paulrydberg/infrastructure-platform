...
| Container/image security scanning | Stage 7A complete: Trivy v0.70.0 image+config scans, gitleaks v8.30.1 history scan, SPDX SBOM, image-reference policy — all CI-side, evidence mode (nothing BLOCKED yet); supply-chain incident (GHSA-69fq-xp46-6x23) documented and bypassed via SHA pins | Experimental (detect+baseline; policy unenforced) | yes (evidence-linked) | 7B policy-enforcement gate |
